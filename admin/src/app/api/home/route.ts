...
export async function PATCH(req: NextRequest) {
  const result = await requirePermissionApi(PERMISSIONS.HOME_MANAGE);
  if (result instanceof NextResponse) return result;

  const body = await req.json().catch(() => null);
  try {
    const page = await homeManagementService.saveDraft(body, result.id);
    return NextResponse.json({ page });
  } catch (err) {
    if (err instanceof ZodError) {
      return NextResponse.json({ error: "بيانات غير صحيحة", issues: err.flatten() }, { status: 400 });
    }
    console.error("Home save draft error:", err);
